...
const Section = ({ crn, partTerm, section, prof, meetingData, seatingData, ...rest }) => {
  ...
  return (
    <Single {...rest}>
      ...
      <List>
        {seatingData?.map(
          ({ max, maxResv, leftResv, enrolled, avail, waitCap, waitCount, waitAvail, roomCap  }, i) => (
            <SeatingItem>
              {/*<ClassItem>Enrolled: {enrolled}</ClassItem>*/}
              <ClassItem>Availability: {avail} / {max}</ClassItem>
              <ClassItem>Reserved: {leftResv} / {maxResv}</ClassItem>
              {/*<ClassItem>Waiting: {waitCount}</ClassItem>*/}
              <ClassItem>Waiting: {waitAvail} / {waitCap}</ClassItem>
